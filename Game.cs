...
void main(){
    FragColor = texture(uTex, vUV);
        ...
        private int TX(float x) => (int)((x + 2f) / 4f * screen.width);

        // Transform world Y coordinate (-2 to 2) to screen Y coordinate (0 to height), inverted Y
        private int TY(float y) => (int)((-y + 2f) / 4f * screen.height);
